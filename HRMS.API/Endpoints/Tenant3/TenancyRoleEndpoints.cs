...
using HRMS.Utility.Validators.Tenant3.TenancyRole;
using Microsoft.AspNetCore.Mvc;

namespace HRMS.API.Endpoints.Tenant3
{
        ...
        public static void MapTenancyRoleEndpoints(this IEndpointRouteBuilder app)
        {
            ...
            app.MapPost("/HRMS/CreateTenancyRole", async (TenancyRoleCreateRequestDto dto, ITenancyRoleService _tenancyroleService) =>
            {
                ...
                catch (Exception ex)
                {
                    return Results.Json(
                        ResponseHelper<string>.Error(
                            message: "An Unexpected Error occurred while Creating the TenancyRole.",
                            exception: ex,
                            isWarning: false,
                            statusCode: StatusCodeEnum.INTERNAL_SERVER_ERROR
                        ).ToDictionary()
            ...
            app.MapDelete("/HRMS/DeleteTenancyRole", async (ITenancyRoleService service, [FromBody] TenancyRoleDeleteRequestDto dto) =>
            {
               ...
               try
               {
                ...
                catch (Exception ex)
                {
                    return Results.Json(
                        ResponseHelper<string>.Error(
                            message: "An Unexpected Error occurred while Deleting the TenancyRole.",
                            exception: ex,
                            isWarning: false,
                            statusCode: StatusCodeEnum.INTERNAL_SERVER_ERROR
                        ).ToDictionary()
